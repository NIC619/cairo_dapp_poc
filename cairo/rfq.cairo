...
func get_order_hash{pedersen_ptr : HashBuiltin*}(
        order : Order*) -> (res : felt):
    let (taker_pubkey_and_id_hash) = hash2{hash_ptr=pedersen_ptr}(
        order.taker_public_key,
        order.taker_token_id)
    ...
    let (maker_pubkey_and_id_hash) = hash2{hash_ptr=pedersen_ptr}(
        order.maker_public_key,
        order.maker_token_id)
...
func fill_order{
    ...
    tempvar taker_token_amount = order.taker_token_amount
    tempvar maker_token_amount = order.maker_token_amount
    ...
    verify_ecdsa_signature(
        message=order_hash,
        public_key=order.taker_public_key,
        signature_r=order.r_a,
        signature_s=order.s_a)
    verify_ecdsa_signature(
        message=order_hash,
        public_key=order.maker_public_key,
        signature_r=order.r_b,
        signature_s=order.s_b)
